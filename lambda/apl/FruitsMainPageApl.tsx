...
class FruitsMainPageApl extends React.Component {
  constructor(props) {
    ...
    this.fruitRows = Utils.splitArray(
      FruitInfo.data,
      this.pageProperties.listRowCount
    )
  }
  render() {
        ...
        <MainTemplate>
          <Container width='100vw' height='100vh'>
            <Image
              ...
              scale='best-fill'
              source={
                FruitSkillMetadata.properties.backgroundImageSourceUrl
              }></Image>
            <Sequence width='100%' height='100%' scrollDirection='vertical'>
              {isViewportSmall ? null : (
                <AlexaHeader
                  headerTitle={FruitSkillMetadata.properties.headerTitle}
                />
              )}
              {this.fruitRows.map(fruitRow => {
                return (
                  <FruitsRow
                    fruitRowData={fruitRow}
